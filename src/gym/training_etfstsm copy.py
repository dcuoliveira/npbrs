...
class training_etfstsm(TSM, DependentBootstrapSampling, Backtest):
    def __init__(self,
    ...
    def objective(self, trial):
        w = trial.suggest_categorical('window', hyperparam_space['window'])

        # for a given window, build signals from bootstrap samples
        self.bootstrap_signals_info = self.build_signals_from_bootstrap_samples(window=w)

        # build forecasts from bootstrap signals
        self.bootstrap_forecasts_info = self.build_forecasts_from_bootstrap_signals()

        # run backtest for each boostrap samples
        utilities = []
        for i in range(self.n_bootstrap_samples):
            # build signals info
            self.signals_info = self.bootstrap_signals_info[f"bootstrap_{i}"]

            # build forecasts info
            self.forecasts_info = self.bootstrap_forecasts_info[f"bootstrap_{i}"]

            # run backtest
            cerebro = Backtest(strat_metadata=strategy)
            cerebro.run_backtest(instruments=self.instruments,
            ...
            utilities.append(metrics["Sharpe"])

        return {"mean": np.mean(utilities), "std": np.std(utilities)}
...
if __name__ == "__main__":
    ...
    # strategy hyperparameter space
    hyperparam_space = {'window': list(range(30, 253))}

    # init sampler
    sampler = optuna.samplers.GridSampler(hyperparam_space)

    study_name = f"{strategy.sysname}_study"
    study = optuna.create_study(study_name=study_name, direction='minimize', sampler=sampler)
    study.optimize(strategy.objective, n_trials=100)