...
def tsmom_signal_from_returns(returns: pd.DataFrame, lookback: int, method: str = "prod"):
    ...
    mom = mom.shift(1)
    return np.sign(mom).replace({-0.0: 0.0})

def macd_signal_from_returns(returns: pd.DataFrame, short_l: int, long_l: int):
    """MACD on returns: EWM(short) - EWM(long), lagged by 1, sign -> {-1,0,1}."""
    fast = returns.ewm(span=short_l, adjust=False).mean()
    slow = returns.ewm(span=long_l, adjust=False).mean()
    macd = (fast - slow).shift(1)
    return np.sign(macd).replace({-0.0: 0.0})

def lag_mom_signal_from_returns(returns: pd.DataFrame, lag: int, diff_from: int | None = None):
    """
    Lagged-returns momentum.
    - If diff_from is None: signal_t = sign( r_{t-lag} )
    - Else: signal_t = sign( r_{t-lag} - r_{t-diff_from} )
    Positions in {-1,0,1}.
    """
    if lag <= 0:
        raise ValueError("lag must be >= 1")
    if diff_from is None:
        sig = returns.shift(lag)
    else:
        sig = returns.shift(lag) - returns.shift(diff_from)
    sig = sig.shift(0)  # already lagged for execution
    return np.sign(sig).replace({-0.0: 0.0})
...
def signal_fn_lag(rets: pd.DataFrame, param: Any) -> pd.DataFrame:
    """
    param may be:
      - int L  -> use sign(r_{t-L})
      - tuple/list (L, D) -> use sign(r_{t-L} - r_{t-D})
    """
    if isinstance(param, (tuple, list)) and len(param) == 2:
        L, D = int(param[0]), int(param[1])
        return lag_mom_signal_from_returns(rets, L, diff_from=D)
    else:
        return lag_mom_signal_from_returns(rets, int(param), diff_from=None)
...
if __name__ == "__main__":
    # ------------------ Choose signal here ------------------
    # Options: "lag_mom", "tsmom_moskowitz_prod", "macd_returns"
    SIGNAL_NAME = "lag_mom"   # <- run momentum using lagged returns as signals
    ...
    # ------------------ Experiment Config -------------------
    if SIGNAL_NAME == "lag_mom":
        signal_fn = signal_fn_lag
        # Simple grid of pure lags (use r_{t-L}); you can also include pairs (L, D) for spread lags
        PARAM_TRIALS: Iterable[Any] = list(range(1, 1 + min(252, len(train_returns)//4)))
        # Example to include spread lags as well:
        # PARAM_TRIALS += [(5, 21), (10, 63), (21, 126)]
    elif SIGNAL_NAME == "tsmom_moskowitz_prod":
    ...
    cum_df = pd.DataFrame(cumret_panels)
