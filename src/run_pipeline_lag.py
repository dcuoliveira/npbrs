...
for name, window in train_sharpe_params.items():
    ...
    # compute signal
    test_signal = test_returns_extended.diff(window).shift(+1)
    signal = returns.diff(window).shift(+1)
...
plt.savefig(os.path.join(outputs_path, 'results', f'{utility.lower()}-cumret-lag-mom.png'), bbox_inches='tight')
